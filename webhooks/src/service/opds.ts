import { ContentType, contentTypeisOpds, contentTypeisOpdsAuth, parseContentType } from "../utils/contentType";
...
export class OpdsService {
    ...
    public async opdsRequest(url: string): Promise<IOpdsResultView> {
        ...
        const res = await httpFetchFormattedResponse<IOpdsResultView>(url, undefined, 
            async (opdsData) => {
    ...
    public async webpubRequest(url: string): Promise<IWebPubView> {
        ...
        const res = await httpFetchFormattedResponse<IWebPubView>(url, undefined,
            async (webpubData) => {
                ...
                if (contentType === ContentType.webpub || contentType === ContentType.Json || contentType === ContentType.JsonLd) {
                    ...
                    if (isR2Pub) {
                        ...
                        webpubData.data = webpubViewConverter.convertWebpubToView(r2Publication, baseUrl);
                    }
    ...
    private async opdsRequestJsonTransformer(
        ...
        if (isAuth) {
            ...
            const pubView = opdsFeedViewConverter.convertOpdsPublicationToView(r2OpdsPublication, baseUrl);
            return {
                title: pubView.title,
            ...
            const pubView = opdsFeedViewConverter.convertOpdsPublicationToView(pub, baseUrl);
            ...
            const { title, publications: _pubs } = opdsFeedViewConverter.convertOpdsFeedToView(r2OpdsFeed, baseUrl);
            return {
                title,
