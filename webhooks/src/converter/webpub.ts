...
export class WebpubViewConverter {
    ...
    public convertWebpubToView(r2Publication: R2Publication, baseUrl: string): IWebPubView {
        ...
        const coverLinks = fallback(
            opdsFeedViewConverter.convertFilterLinksToView(baseUrl, r2Publication.Resources, {
        ...
        const readingOrdersLinks = opdsFeedViewConverter.convertFilterLinksToView(baseUrl, r2Publication.Spine, {
            type: ["audio/mpeg"],
        });

        const readingOrders = readingOrdersLinks.map<IReadingLink>((l) => ({
            duration: l.duration,
            url: l.url,
        })) || [];
