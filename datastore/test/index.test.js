...
const execName = 'env GOOGLE_APPLICATION_CREDENTIALS=$PWD/../service_account.json functions-framework';
...
describe('functions/datastore', () => {
  describe('set', () => {
    ...
    before(async () => {
      ffProc = execPromise(
        `${execName} --target=set --signature-type=http --port=${PORT}`,
  ...
  describe('get', () => {
    ...
    before(async () => {
      ffProc = execPromise(
        `${execName} --target=get --signature-type=http --port=${PORT}`,
        {timeout: FF_TIMEOUT, shell: true, cwd}
      );
      await waitForReady(BASE_URL);
    });
  ...
  describe('del', () => {
    ...
    before(async () => {
      ffProc = execPromise(
        `${execName} --target=del --signature-type=http --port=${PORT}`,
        {timeout: FF_TIMEOUT, shell: true, cwd}
      );
      await waitForReady(BASE_URL);
    });
